...
module.exports = function () {
    ...
    this.addDataOnPacks = async function (data, options) {
        ...
        let arr = [];
            ...
            arr = data.metarials
            let temp = await Packs.findOneAndUpdate({_id:packid}, {$push : {metarials : arr}},{new:true})
            return temp
            }
            else if(data.colors && data.colors.length > 0){
                arr = data.colors
                let temp = await Packs.findOneAndUpdate({_id:packid}, {$push : {colors : arr}},{new:true})
                return temp
            }else if(data.patterns && data.patterns.length > 0){
                arr = data.patterns
                let temp = await Packs.findOneAndUpdate({_id:packid}, {$push : {patterns : arr}},{new:true})
                return temp
            }
    }catch(e){
        console.log(e)
    ...
    this.editDataOnPacks = async function(data, options){
        ...
        if(data.metarials && data.metarials.length > 0){
        let temp = await Packs.updateOne({_id: packid, metarials : {$elemMatch : {_id: arrid}}}, {$set : {"metarials.$" : data.metarials[0]}});
            return temp
        }
        else if(data.colors && data.colors.length > 0){
            let temp = await Packs.updateOne({_id: packid, metarials : {$elemMatch : {_id: arrid}}}, {$set : {"colors.$" : data.metarials[0]}});
            return temp
        }
        else if(data.patterns && data.patterns.length > 0){
            let temp = await Packs.updateOne({_id: packid, metarials : {$elemMatch : {_id: arrid}}}, {$set : {"patterns.$" : data.metarials[0]}});
            return temp
        }
    }catch(e){
        throw e
    ...
    this.deleteMaterial =  async function (data, options){
        try{
            ...
            let arrid = data.arrId;
                ...
                let temp = await Packs.findByIdAndUpdate({_id:packid}, {$pull : {metarials : {_id : arrid}}},{new:true})
                return temp
                }
                else if(data.type ==  "COLORS"){
                    let temp = await Packs.findOneAndUpdate({_id:packid}, {$pull : {colors : {_id : arrid}}},{new:true})
                    return temp
                }else if(data.type ==  "PATTERNS"){
                    let temp = await Packs.findOneAndUpdate({_id:packid}, {$pull : {patterns : {_id : arrid}}},{new:true})
                    return temp
                }
        }catch(e){
            console.log(e)
    ...
    this.setVisible =  async function (data, options){
        try{
            ...
            let arrid = data.arrId;
                if(data.type ==  "MATERIALS"){
                    let temp = await Packs.updateOne({_id: packid, metarials : {$elemMatch : {_id: arrid}}}, {$set : {"metarials.$.visible" : data.visible}});
                        return temp
                }
                else if(data.type ==  "COLORS"){
                    let temp = await Packs.updateOne({_id: packid, metarials : {$elemMatch : {_id: arrid}}}, {$set : {"colors.$.visible" : data.visible}});
                    return temp
                }else if(data.type ==  "PATTERNS"){
                    let temp = await Packs.updateOne({_id: packid, metarials : {$elemMatch : {_id: arrid}}}, {$set : {"patterns.$.visible" : data.visible}});
                    return temp
                }
        }catch(e){
            console.log(e)
